...
if(if_ready && !hasDamaged)
{
	...
	if(instance_exists(hitUnit) && distance_to_object(hitUnit) < speed)
	{
		...
		if(hitUnit.type != "asteroid" && (!hitUnit.hasShield || hitUnit.shieldHealth <= 0))
		{
			
			array_insert(hitUnit.effects, 0, ["Decay", effect_dmg, 8, Ark_Dappist_Daughter, faction_number]);
		}
	...
	else if(instance_exists(target) && target.faction_number != faction_number && distance_to_object(target) < speed)
	{
		...
		if(target.type != "asteroid" && (!target.hasShield || target.shieldHealth <= 0))
		{
			
			array_insert(target.effects, 0, ["Decay", effect_dmg, 8, Ark_Dappist_Daughter, faction_number]);
		}
...
var travelledDist = distance_to_point(create_x, create_y);
 
if(if_ready && travelledDist > range)
{
	instance_destroy();
