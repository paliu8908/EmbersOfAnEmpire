...
if(hp >= 0 && !stun)
{
	...
	//rotate turret
	for(var _i = 0; _i < array_length(main_weapon_list) && instance_exists(main_weapon_list[_i][12]); _i++)
	{
		...
		if(abs(_angleDiff) > _wpnFireAngle)
		{
		
			if(abs(_angleDiff) < turretRotation)
			{
...
/*	else if(if_move == 2)
	...
	//Main Weapon, run this loop per weapon
	for(var _i = 0; _i < array_length(main_weapon_list); _i++)
	{
		...
		{
			ds_list_delete(targetList, ds_list_find_index(targetList, _wpnTarget));
		}
		...
		if(atkOrder)
		{
			if(instance_exists(target))
			{
				...
				if(_wpnReload != 0 && _wpnBurst == 0 && enemyDis < range &&  angleDifference <= main_weapon_list[_i][2])
				{
			...
			//Choose a new target because target destroyed, acquiring target
			else if(_wpnIfShoot)
			{
				//New target if the target is destroyed, drawn from queue
		...
		//Find new target, acquire from around
		else
		{
			...
			if(!instance_exists(_wpnTarget) || _wpnIfShoot == false || distance_to_object(_wpnTarget) >= _wpnRange || !_inAngle)
			{
				...
				if(ds_list_size(enemies) != 0)
				{
					...
					for(var choices = 0; choices < enemy_numbers; choices++;)
					{
						...
						if(instance_exists(_wpnTarget) && _wpnTarget.faction_number != self.faction_number && distance_to_object(_wpnTarget) < _wpnRange && _wpnTarget.type != "asteroid" && ds_list_find_index(targetList, _wpnTarget) == -1)
						{
							...
							ds_list_add(targetList, _wpnTarget);

						}
						else
						{
							var i = ds_list_find_index(targetList, _wpnTarget);
