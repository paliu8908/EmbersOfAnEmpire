...
for(var a = 0; a < 4; a++)
{
	var mate = instance_create_layer(1000 + a%4 * 75, 1000 + 100 * floor(a/4), "instances", Prox_Mech_Gunner);
	mate.faction_number = ds_list_find_index(global.factions, "player");
	
	//mate.skill_points += 12;
...
for(var e = 0; e < 6; e++)
{
	var _uni = instance_create_layer(3000 -  e % 4 * 100, 3000 - 100* floor(e/4), "instances", Ark_Corot_Corvette);
	
	_uni.faction_number = ds_list_find_index(global.factions, "ai");
	
	//ds_list_add(global.battlegroup1, _uni);
	
	_uni.target = Prox_Mech_Gunner.id;
